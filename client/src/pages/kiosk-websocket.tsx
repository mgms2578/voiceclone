...
export default function KioskPage() {
  ...
  const handleVoiceInput = () => {
    // ✅ 0. 이번 턴에는 "내가 말한 다음에 오는 응답"은 읽어도 되지만,
    //    지금 재생 중인 음성은 무조건 끊자
    ttsAllowedRef.current = true;

    console.log("마이크 버튼 클릭 → TTS 강제 중단 시도");

    // 1차 정지: 바로 TTS 중단
    tts.stop();
    speech.setTTSActive(false);

    // 🔁 2차 정지: 짧은 딜레이 후 한 번 더 강제 중단
    setTimeout(() => {
      console.log("마이크 버튼 → 2차 TTS 정지");
      try {
        tts.stop();
        stopAllGlobalAudio(); // (HTMLAudio 기반 재생이 있다면 이것도 같이 정지)
      } catch (e) {
        console.log("TTS 2차 정지 중 오류:", e);
      }
    }, 150); // 100~200ms 정도면 충분

    // 2. 이후 마이크 on/off 토글
    if (speech.isListening) {
      console.log("마이크 버튼: 듣기 중 → stopListening");
      speech.stopListening();
      speech.resetTranscript(); // 남아있던 텍스트도 제거 (autoSend 방지)
    } else {
        ...
        <div className="max-w-4xl mx-auto h-full flex flex-col md:justify-center">
          ...
          <div className="flex gap-2 md:gap-6 justify-center mb-2 md:mb-6 consent-buttons px-2">
            <Button
              ...
              variant="outline"
              className="text-sm md:text-2xl font-semibold py-2.5 md:py-6 px-4 md:px-14 whitespace-nowrap flex-1 h-auto md:min-h-[120px]"
            ...
            <Button
              ...
              className="bg-blue-600 text-sm md:text-2xl font-semibold py-2.5 md:py-6 px-6 md:px-14 whitespace-nowrap flex-1 h-auto md:min-h-[120px]"
              disabled={createSessionMutation.isPending}
