...
export default function KioskPage() {
  ...
  const tts = useTTS({ mode: 'download', sessionId });
  ...
  const sendMessageMutation = useMutation({
    mutationFn: async (content: string) => {
      ...
      const response = await apiRequest('POST', `/api/download/sessions/${sessionId}/messages`, {
        content,
        role: 'user',
      });
      return response.json();
    ...
    onSuccess: (data: { userMessage: Message; aiMessage: Message }) => {
      ...
      // 자동재생 복구 - AI 응답시 자동으로 음성 재생
      if (data.aiMessage.audioUrl) {
        const audio = new Audio(data.aiMessage.audioUrl);
        
        // 전역 오디오 추적에 추가
        addGlobalAudio(audio);
        
        // TTS 재생 완료 후 상태 정리
        audio.onended = () => {
          speech.setTTSActive(false);
          removeGlobalAudio(audio);
          // 사용자가 원하면 마이크 버튼을 눌러서 다시 활성화 가능
        };
  ...
  useEffect(() => {
    if (currentStep === 'chat' && messages.length === 0 && sessionId) {
      // Request welcome message from server (Download version)
      fetch(`/api/download/sessions/${sessionId}/welcome`, { method: 'POST' })
        .then(res => res.json())
        .then(data => {
          if (data.message && data.audioUrl) {
            ...
            audio.play().catch((error) => {
              console.error('Welcome audio play error:', error);
              speech.setTTSActive(false);
              removeGlobalAudio(audio);
            });
          }
  ...
  if (currentStep === 'intro') {
    // 인트로 화면
    return (
      <div className="h-screen w-screen flex flex-col items-center justify-center bg-gradient-to-br from-blue-600 to-purple-700 text-white kiosk-intro">
        ...
        <div className="bg-blue-600 text-white p-6">
          <div className="max-w-6xl mx-auto flex items-center justify-between">
            <div>
              ...
              <p className="opacity-90">복제된 목소리로 딥페이크 위험성에 대해 대화해보세요</p>
            </div>
