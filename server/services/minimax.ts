...
export async function cloneVoice({
  audioBuffer,
  sessionId,
}: CloneVoiceOptions): Promise<MinimaxResponse> {
  ...
  try {
    return await simpleRetry(async () => {
      ...
      const uploadResponse = await fetch(
        `https://api.minimax.io/v1/files/upload?GroupId=${groupId}`,
        {
          method: "POST",
          headers: {
      ...
      if (!uploadResponse.ok) {
        ...
        const error: any = new Error(
          `파일 업로드 실패: ${uploadResponse.status}`,
        );
      ...
      const cloneResponse = await fetch(
        `https://api.minimax.io/v1/voice_clone?GroupId=${groupId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${apiKey}`,
      ...
      if (!cloneResponse.ok) {
        ...
        const error: any = new Error(
          `음성 클로닝 실패: ${cloneResponse.status}`,
        );
      ...
      if (cloneData.base_resp?.status_code !== 0) {
        throw new Error(
          `음성 클로닝 오류: ${cloneData.base_resp?.status_msg || "알 수 없는 오류"}`,
        );
      }

      // Use the returned voice_id or fallback to our generated one
      const actualVoiceId =
        cloneData.voice_id || cloneData.data?.voice_id || voiceId;
  ...
  try {
    return await simpleRetry(async () => {
      ...
      if (!deleteResponse.ok) {
        ...
        const error: any = new Error(
          `클론 삭제 실패: ${deleteResponse.status}`,
        );
    ...
    return await simpleRetry(async () => {
      ...
      try {
        ...
        if (getVoiceResponse.ok) {
          ...
          console.log(
            "MiniMax get-voice response:",
            JSON.stringify(data, null, 2),
          );

          if (data.base_resp && data.base_resp.status_code === 1004) {
            console.log(
              "API authentication failed - valid MINIMAX_API_KEY required",
            );
          ...
          console.log(
            `get-voice API failed: ${getVoiceResponse.status} - ${errorText}`,
          );
          const error: any = new Error(
            `get-voice API failed: ${getVoiceResponse.status}`,
          );
        ...
        if (!response.ok) {
          ...
          const fallbackError: any = new Error(
            `MiniMax Voice List 오류: ${response.status}`,
          );
  ...
  try {
    return await simpleRetry(async () => {
      ...
      if (data.base_resp?.status_code !== 0) {
        throw new Error(
          `음성 생성 오류: ${data.base_resp?.status_msg || "알 수 없는 오류"}`,
        );
      }
...
export async function cloneVoiceCompat(
  audioBuffer: Buffer,
